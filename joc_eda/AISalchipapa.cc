...
int closestEnemy(const Pos& myPos) {
  ...
  Dir scape(const Dir& enemy, const Pos& currentPos) {
    ...
    if (it != oppositeDirs.end()) {
        ...
        if (!escapeDirs.empty()) {
            for (int i = 0; i <6; ++i) {
                ...
                if (pos_ok(escapePos) && cell(escapePos).type != Rock && !isMine(escapePos) && !isEnemy(escapePos)) {
  ...
  void move_pioneers(){
    ...
    for (int i = 0; i < mida ; ++i) {
        ...
        if(res == None){
           ...
           command(myPioneers[i],path[0]);
          }
        else{
          res = scape(res,unit(myPioneers[i]).pos);
          
          command(myPioneers[i],res);
        } 
          }
  ...
  void move_furyans(){
    ...
    for (int i = 0; i < mida; ++i) {
      ...
      if(res == None){
          ...
          command(myFuryans[i],path[0]);
        }
      else{
        res = scape(res,unit(myFuryans[i]).pos);
          
        command(myFuryans[i],res);
      }
